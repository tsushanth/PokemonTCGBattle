...
def _determinize(obs: Observation, deck: list[int]) -> dict:
    ...
    for cid in known_you:
        if cid in pool:
            pool.remove(cid)
    random.shuffle(pool)
    pool = (pool + deck)[:remaining_needed] if len(pool) < remaining_needed else pool[:remaining_needed]
    ...
    if opp.active and opp.active[0] is None:
        opponent_active = [random.choice([c for c in all_ids])]
...
def _choose_main(obs: Observation, deck: list[int]) -> list[int]:
    ...
    if all(s is None for s in scores):
        _log(f"MAIN: search unusable for all {n} options, using no-lookahead priority rule")
        return _greedy_choice(obs)

    best_idx = max(range(n), key=lambda i: (scores[i] if scores[i] is not None else -1e9))
    _log(f"MAIN: {n} options, rollout_steps_total={total_steps}, "
